...
const char OV2640_QQVGA[][2] = {
    ...
    {0x13, 0xe5},  // Common control 8 => Banding filter on, auto AGC, auto exposure...? Should be 0xe7 since bit1 is
                   // reserved
 ...
 *
 */
int8_t ov2640_set_bank(uint8_t bank) { return write_reg(OV2640_ADDR, OV2640_DSP_RA_DLMT, bank); }
...
int8_t ov2640_read_id(uint16_t* id) {
    ...
    if ((err = read_reg(OV2640_ADDR, OV2640_SENSOR_PIDH, &regValue[0])) != HAL_OK) {
        return err;
    }
    if ((err = read_reg(OV2640_ADDR, OV2640_SENSOR_PIDL, &regValue[1])) != HAL_OK) {
        return err;
    }
...
int8_t ov2640_set_qqvga(void) {
    ...
    for (i = 0; i < (sizeof(OV2640_QQVGA) / 2); i++) {
        if ((err = write_reg(OV2640_ADDR, OV2640_QQVGA[i][0], OV2640_QQVGA[i][1])) != HAL_OK) {
            return err;
        }
    }
