...
struct po6030_configuration {
    ...
    int curr_subsampling_x;
    int curr_subsampling_y;
};
 ...
 *          These values are considered only when auto white balance is disabled, so this function also disables auto
 * white balance. The resulting gain is the value divided by 64 (max resulting gain = 4).
 *
 ...
 * 			These values are considered only when auto exposure is disabled, so this function also disables
 * auto exposure. Total integration time is: (integral + fractional/256) * line time.
 *
