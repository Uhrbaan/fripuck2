...
- 300µs: Channel 1 gets activated which triggers the callback `HAL_TIM_OC_DelayElapsedCallback` and turns off the IR
lights.

The data is read twice (two cycles) for each pin, but in the first cycle, the ir light is on, and on the second cycle,
