...
int register_sensor(float priority, float age_step, pack_fn pack_fn) {
    struct sensor_entry entry = {.priority = priority, .age = 0, .age_step = age_step, .pack_f = pack_fn};

    int i = 0;
    for (; i < SENSOR_LIST_SIZE; i++) {
        if (sensor_list[i].pack_f != NULL) {
            sensor_list[i] = entry;
            if (i > last_sensor_index) last_sensor_index = i;
...
int unregister_sensor(int sensor_index) {
    ...
    // update last index if necessary, go back as many consecutive NULLs from the end (less expensive than moving
    // memory)
    if (sensor_index >= last_sensor_index) {
        int i = last_sensor_index;
        for (; i >= 0; i--) {
            if (sensor_list[i].pack_f != NULL) break;
        }
        last_sensor_index = i;
    }
}
