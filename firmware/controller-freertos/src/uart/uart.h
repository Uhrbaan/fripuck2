 ...
 * @param function The function that will be called when a message is recieved from the radio chip. It should be of the
 * shape `typedef void (*uart_callback_fn)(uint8_t *data, uint16_t length);`
 */
 ...
 *         - 2 (or `HAL_BUSY`) if the UART connection is currently busy (could happen if you tried to send two messages
 * at to close intervals. Try to wait a millisecond and try again.)
