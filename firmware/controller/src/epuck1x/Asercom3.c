...
enum active_sensors {
    // Sensors from the IMU. You can find the product specifications here:
    // https://projects.gctronic.com/epuck2/doc/MPU-9250-product-specification.pdf
    // Note that the IMU comes with a DMP (Digital Motion Sensor), which could be really useful data.
    // TODO : look into it, if it's even possible to access this data.
    //
    // All of these are stored as 16-bit numbers.
    active_sensor_IMU_acceleration_x = 1 << 0, ///< Acceleration on the X-axis
    active_sensor_IMU_acceleration_y = 1 << 1, ///< Acceleration on the Y-axis
    active_sensor_IMU_acceleration_z = 1 << 2, ///< Acceleration on the Z-axis
    active_sensor_IMU_gyroscope_x = 1 << 3,    ///< Gyroscope on the X-axis
    active_sensor_IMU_gyroscope_y = 1 << 4,    ///< Gyroscope on the Y-axis
    active_sensor_IMU_gyroscope_z = 1 << 5,    ///< Gyroscope on the Z-axis
    active_sensor_IMU_magnetometer_x = 1 << 6, ///< Magnetometer on the X-axis
    active_sensor_IMU_magnetometer_y = 1 << 7, ///< Magnetometer on the Y-axis
    active_sensor_IMU_magnetometer_z = 1 << 8, ///< Magnetometer on the Z-axis

    // Temperature
    // Stored as a singly byte uint8_t
    active_sensor_temperature = 1 << 9, ///< Temperature sensor

    // Proximity sensors
    // All the values are stored as uint32_t
    // TODO: compress the data since it is quite large.
    active_sensor_PROX_ambient_0 = 1 << 10, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_0 = 1 << 11,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_1 = 1 << 12, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_1 = 1 << 13,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_2 = 1 << 14, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_2 = 1 << 15,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_3 = 1 << 16, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_3 = 1 << 17,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_4 = 1 << 18, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_4 = 1 << 19,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_5 = 1 << 20, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_5 = 1 << 21,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_6 = 1 << 22, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_6 = 1 << 23,   ///< Difference between ambient and reflected light picked up by proximity sensors.
    active_sensor_PROX_ambient_7 = 1 << 24, ///< Ambient light picked up by proximity sensors.
    active_sensor_PROX_delta_7 = 1 << 25,   ///< Difference between ambient and reflected light picked up by proximity sensors.

    // Battery level.
    // Stored as a single uint32_t, maybe could loose some precision without problems.
    active_sensor_battery = 1 << 26, ///< Value of the current battery level

    // Selector.
    // Stored as a single uint8_t
    active_sensor_selector_position = 1 << 27, ///< Current state of the selector, 0-F

    // Infra-Red receiver
    // Probably can pack all three in a single value
    active_sensor_ir_receiver = 1 << 28, ///< Get data from remote-control

    // Ground sensors
    active_sensor_GROUND_delta_0 = 1 << 29,   ///< Ambient light picked up by ground sensors.
    active_sensor_GROUND_ambient_0 = 1 << 30, ///< Difference between ambient and reflected light picked up by ground sensors.
    active_sensor_GROUND_delta_1 = 1 << 31,   ///< Ambient light picked up by ground sensors.
    active_sensor_GROUND_ambient_1 = 1 << 32, ///< Difference between ambient and reflected light picked up by ground sensors.
    active_sensor_GROUND_delta_2 = 1 << 33,   ///< Ambient light picked up by ground sensors.
    active_sensor_GROUND_ambient_2 = 1 << 34, ///< Difference between ambient and reflected light picked up by ground sensors.

    // Camera
    active_sensor_CAMERA_active = 1 << 35, ///< Set to 1 if the camera is active.
    /**
     * @brief Bits for selecting the camera's color space.
     *
     * Two bits needed to select the camera's color space. The robot will support RGB (0), HSL (1) and OkLab (2). OkLab
     * is an interesting one since it provides a very hight quality "luminescance" channel.
     *
     */
    active_sensor_CAMERA_colorspace_start_bit = 1 << 36,
    /**
     * @brief Bits per channel for the first color channel.
     *
     * First bit of the four bits that store the number of of bits used for the first channel.
     * Channel0 corresponds to the "Red" channel in the RGB color space, the "Hue" channel in the HSL color space, and
     * the "Luminescence" channel in the OkLab color space.
     *
     * These four bits encode the number of bits used per color channel. 0 means it is disabled, and 8 is the maximum.
     *
     */
    active_sensor_CAMERA_channel0_bpc_start_bit = 1 << 38,
    /**
     * @brief Bits per channel for the second color channel.
     *
     * First bit of the four bits that store the number of of bits used for the second channel.
     * Channel1 corresponds to the "Green" channel in the RGB color space, the "Saturation" channel in the HSL color
     * space, and the "chroma" channel ($a$, which represents the "intensity" of the color) in the OkLab color space.
     *
     * These four bits encode the number of bits used per color channel. 0 means it is disabled, and 8 is the maximum.
     *
     */
    active_sensor_CAMERA_channel1_bpc_start_bit = 1 << 42,
    /**
     * @brief Bits per channel for the third color channel.
     *
     * First bit of the four bits that store the number of of bits used for the third channel.
     * Channel2 corresponds to the "Blue" channel in the RGB color space, the "Lightness" channel in the HSL color
     * space, and the "hue" channel ($b$, which represents the hue angle of the color) in the OkLab color space.
     *
     * These four bits encode the number of bits used per color channel. 0 means it is disabled, and 8 is the maximum.
     *
     */
    active_sensor_CAMERA_channel2_bpc_start_bit = 1 << 46,
};

#define CAMERA_COLORSPACE_WIDTH 2
#define CAMERA_COLORSPACE_MASK ((1U << CAMERA_COLORSPACE_WIDTH) - 1)
#define CAMERA_BPC_WIDTH 4
#define CAMERA_BPC_MASK ((1U << CAMERA_BPC_WIDTH) - 1)
...
int asercom_get_help_message_ascii_H(int buffer_offset, BaseSequentialStream *uart_target) {
    ...
    chSequentialStreamWrite(uart_target, "\"J,#,#,#,#,#,#\"   Set camera parameter mode,width,heigth,zoom(1,4 or 8),x1,y1\r\n", 80);
...
asercom_camera_parameter_t asercom_camera_parameters = {.mode = RGB_565_MODE, .width = 40, .heigth = 40, .zoom = 8, .size = 40 * 40 * 2};

// TODO: add camera information
int asercom_get_camera_info_ascii_I(int buffer_offset, BaseSequentialStream *uart_target) {
    sprintf(buffer, "i,%d,%d,%d,%d,%d\r\n", &asercom_camera_parameters.mode, &asercom_camera_parameters.width, &asercom_camera_parameters.heigth,
            &asercom_camera_parameters.zoom, &asercom_camera_parameters.size);
    chSequentialStreamWrite(uart_target, buffer, strlen(buffer));
    return 0;
}
...
int asercom_set_camera_parameter_ascii_J(int buffer_offset, BaseSequentialStream *uart_target) {
    ...
    sscanf(buffer, "J,%d,%d,%d,%d,%d,%d\r", &asercom_camera_parameters.mode, &asercom_camera_parameters.width, &asercom_camera_parameters.heigth,
           &asercom_camera_parameters.zoom, &asercom_camera_parameters.x1, &asercom_camera_parameters.y1);
    ...
    if (asercom_camera_parameters.size > IMAGE_MAX_SIZE) { // if desired settings too demanding set to a reasonable default
    ...
    if (asercom_camera_parameters.y1 == -1) { // user did not specify: take default
        asercom_camera_parameters.y1 = (ARRAY_WIDTH - asercom_camera_parameters.width * asercom_camera_parameters.zoom) / 2;
    }
    if (asercom_camera_parameters.y1 == -1) { // user did not specify: take default
        asercom_camera_parameters.y1 = (ARRAY_HEIGHT - asercom_camera_parameters.heigth * asercom_camera_parameters.zoom) / 2;
    }
    e_poxxxx_config_cam(asercom_camera_parameters.y1, asercom_camera_parameters.y1, asercom_camera_parameters.width * asercom_camera_parameters.zoom,
                        asercom_camera_parameters.heigth * asercom_camera_parameters.zoom, asercom_camera_parameters.zoom, asercom_camera_parameters.zoom,
                        asercom_camera_parameters.mode);
