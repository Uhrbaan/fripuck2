...
def serial_ports():
	...
	for port in ports:
		...
		if port.vid==7504 and port.pid==24600: # Put in a list only the ports related to the programmer
			...
			#print("product = " + str(port.product))
			#print("interface = " + str(port.interface))
			#print("usb_description = " + str(port.usb_description()))
			#print("usb_info = " + str(port.usb_info()))
			#print("\r\n")
			#print(dir(port))
	...
	if len(epuck_ports) == 2:
		...
		if(epuck_ports[0].location == None) and (epuck_ports[1].location == None):
			print("Cannot find the correct port!")
		elif(epuck_ports[0].location == None):
			if(int(epuck_ports[1].location[-1]) == 2):
				print(str(epuck_ports[0].device))
			else:
				print(str(epuck_ports[1].device))
		elif(epuck_ports[1].location == None):	
			if(int(epuck_ports[0].location[-1]) == 2):
				print(str(epuck_ports[1].device))
			else:
				print(str(epuck_ports[0].device))
		else:	
			if int(epuck_ports[0].location[-1]) < int(epuck_ports[1].location[-1]):
				print(str(epuck_ports[0].device))
			else:
				print(str(epuck_ports[1].device))
	#if len(epuck_ports) == 2:
	#	print(str(epuck_ports[0].device) + ", " + str(epuck_ports[1].device))
