...
static void cmd_readclock(BaseSequentialStream *chp, int argc, char *argv[]) {
    (void)argc;
    (void)argv;
    chprintf(chp, "SYSCLK: %i \n HCLK: %i \n PCLK1  %i \n PCLK2 %i \n", STM32_SYSCLK, STM32_HCLK, STM32_PCLK1,
             STM32_PCLK2);
}
...
static void cmd_cam_set_brightness(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 1) {
        chprintf(chp, "Usage: cam_brightness value.\r\nDefault=0, max=127, min=-128.\r\n");
    } else {
...
static void cmd_cam_set_contrast(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 1) {
        chprintf(chp, "Usage: cam_contrast value.\r\nDefault=64, max=255, min=0.\r\n");
    } else {
        value = (int8_t)atoi(argv[0]);
...
static void cmd_cam_set_conf1(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 3) {
        chprintf(chp, "Usage: cam_conf1 format subsampling_x subsampling_y\r\nformat: 0=color, 1=grey\r\nsubsampling: "
                      "1, 2, 4\r\n");
    } else {
...
static void cmd_cam_set_mirror(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 2) {
        chprintf(chp, "Usage: cam_mirror vertical_en horizontal_en\r\n1=enabled, 0=disabled\r\n");
    } else {
...
static void cmd_cam_set_gain(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 3) {
        chprintf(chp, "Usage: cam_gain red_gain green_gain blue_gain\r\nThis command disable auto white "
...
static void cmd_cam_set_awb(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 1) {
        chprintf(chp, "Usage: cam_awb awb_en.\r\n1=enabled, 0=disabled\r\n");
    } else {
...
static void cmd_cam_set_ae(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 1) {
        chprintf(chp, "Usage: cam_ae ae_en.\r\n1=enabled, 0=disabled\r\n");
    } else {
...
static void cmd_cam_set_exposure(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if (argc != 2) {
        chprintf(chp, "Usage: cam_exposure integral fractional\r\nUnit is line time; total integration time = "
                      "(integral + fractional/256) line time.\r\nDefault: integral=128, fractional=0\r\n");
    } else {
...
static void cmd_audio_play(BaseSequentialStream *chp, int argc, char *argv[]) {
    uint16_t freq;
    if (argc != 1) {
        chprintf(chp, "Usage: audio_play freq\r\nfreq=100..20000 Hz\r\n");
    } else {
...
void cmd_sdc(BaseSequentialStream *chp, int argc, char *argv[]) {
    ...
    if ((strcmp(argv[0], "read") == 0) || (strcmp(argv[0], "all") == 0)) {
    ...
    if ((strcmp(argv[0], "write") == 0) || (strcmp(argv[0], "all") == 0)) {
        unsigned i;
    ...
    if ((strcmp(argv[0], "erase") == 0) || (strcmp(argv[0], "all") == 0)) {
        /**
         * Test sdcErase()
         * Strategy:
        ...
        /* 3.2. */
        if (memcmp(buf + MMCSD_BLOCK_SIZE, buf2 + MMCSD_BLOCK_SIZE, MMCSD_BLOCK_SIZE) == 0) {
...
const ShellCommand shell_commands[] = {{"mount", cmd_mount},
                                       {"unmount", cmd_unmount},
...
static THD_FUNCTION(shell_spawn_thd, p) {
    ...
    static const ShellConfig shell_cfg = {(BaseSequentialStream *)&SDU1, shell_commands};
