// This file contains the old code for different demos.
// Original code by GTronic, with very minor modifications.
...
void RaB_demo() {
    switch (rab_state) {
    case 0:
        proximity_stop(); // Stop proximity sampling to avoid general interference on all sensors of the range and
                          // bearing extension.
    ...
    case 1:                                                              // Receiver mode
        ...
        if (regValue[0] != 0) {
            ...
            memset(rab_buff, 0x00, 35);
            sprintf((char *)rab_buff, "%d %3.2f %d %d\r\n", rab_data, (rab_bearing * 180.0 / M_PI), rab_range,
                    rab_sensor);
            chSequentialStreamWrite(&SD3, rab_buff, strlen((char *)rab_buff));
            if (SDU1.config->usbp->state == USB_ACTIVE) { // Skip printing if port not opened.
                chprintf((BaseSequentialStream *)&SDU1, "%d %3.2f %d %d\r\n", rab_data, (rab_bearing * 180.0 / M_PI),
                         rab_range, rab_sensor);
            }
        }
...
void RaB_clustering_demo() {
    while (1) {
        ...
        case 0:
            ...
            proximity_start(SLOW_UPDATE); // Start proximity sampling at a lower rate to avoid too much interference
                                          // with the "range and bearing" sensors.
        ...
        case 1:
            write_reg(rab_addr, 13, rab_tx_data >> 8);
            proximity_stop(); // Stop the proximity sampling to avoid interference with the message sent with the range
                              // and bearing extension. This increases the probability that the message will be received
                              // correctly by others extensions.
            write_reg(rab_addr, 14, rab_tx_data & 0xFF);
            ...
            if (regValue[0] != 0) {
                ...
                if (SDU1.config->usbp->state == USB_ACTIVE) { // Skip printing if port not opened.
                    chprintf((BaseSequentialStream *)&SDU1, "%d %3.2f %d %d\r\n", rab_data,
                             (rab_bearing * 180.0 / M_PI), rab_range, rab_sensor);
                }
                if (rab_data != rab_tx_data) { // Stop moving when receiving something from another robot to form a
                                               // cluster. Avoid to stop when receiving bouncing transmitted data.
        ...
        proximity_start(
            SLOW_UPDATE); // Restart proximity sampling at a lower rate only after 11 ms to let the message be
                          // transmitted completely (included a small pause) by the range and bearing extension.
...
void local_communication_demo() {
    proximity_stop();
    startTransceiver();
}

void obstacle_avoidance_demo() {
    while (1) {
        messagebus_topic_wait(prox_topic, &prox_values, sizeof(prox_values));
        ...
        rightSpeed =
            MOTOR_SPEED_LIMIT / 2 - prox_values_temp[7] * 8 - prox_values_temp[6] * 4 - prox_values_temp[5] * 2;
...
void magnetometer_demo() {
    while (1) {
        ...
        case 0: // Setup and calibrate the magnetometer.
            ...
            messagebus_topic_wait(imu_topic, &imu_values, sizeof(imu_values));
            if (SDU1.config->usbp->state == USB_ACTIVE) { // Skip printing if port not opened.
                chprintf((BaseSequentialStream *)&SDU1, "adj_x=%f adj_y=%f adj_z=%f\r\n", imu_values.mag_sens_adjust[0],
                         imu_values.mag_sens_adjust[1], imu_values.mag_sens_adjust[2]);
                ...
                chprintf((BaseSequentialStream *)&SDU1, "scale_x=%f scale_y=%f scale_z=%f\r\n", imu_values.mag_scale[0],
                         imu_values.mag_scale[1], imu_values.mag_scale[2]);
            }
            magneto_state = 1;
            break;
        ...
        case 1:                                                                // Compute the heading.
            ...
            if (SDU1.config->usbp->state == USB_ACTIVE) { // Skip printing if port not opened.
                chprintf((BaseSequentialStream *)&SDU1, "Mx=%f My=%f Mz=%f\r\n", imu_values.magnetometer[0],
                         imu_values.magnetometer[1], imu_values.magnetometer[2]);
            ...
            chThdSleepUntilWindowed(time,
                                    time + MS2ST(125)); // Refresh @ 8 Hz, that is the magnetometer update frequency.
            break;
        }
    }
}
    ...
    while (1) {
        ...
        case 1:                                              // Test.
            ...
            chprintf((BaseSequentialStream *)&SDU1, "Ax=%-7d Ay=%-7d Az=%-7d Gx=%-7d Gy=%-7d Gz=%-7d\r\n\n",
                     imu_values.acc_raw[0], imu_values.acc_raw[1], imu_values.acc_raw[2], imu_values.gyro_raw[0],
                     imu_values.gyro_raw[1], imu_values.gyro_raw[2]);
            ...
            chprintf((BaseSequentialStream *)&SDU1, "%4d,%4d,%4d,%4d,%4d,%4d,%4d,%4d\r\n\n", prox_values.delta[0],
                     prox_values.delta[1], prox_values.delta[2], prox_values.delta[3], prox_values.delta[4],
                     prox_values.delta[5], prox_values.delta[6], prox_values.delta[7]);
            chprintf((BaseSequentialStream *)&SDU1, "AMBIENT\r\n");
            chprintf((BaseSequentialStream *)&SDU1, "%4d,%4d,%4d,%4d,%4d,%4d,%4d,%4d\r\n\n", prox_values.ambient[0],
                     prox_values.ambient[1], prox_values.ambient[2], prox_values.ambient[3], prox_values.ambient[4],
                     prox_values.ambient[5], prox_values.ambient[6], prox_values.ambient[7]);

            // Read microphones.
            chprintf((BaseSequentialStream *)&SDU1, "MICROPHONES\r\n");
            chprintf((BaseSequentialStream *)&SDU1, "%4d,%4d,%4d,%4d\r\n\n", mic_get_volume(0), mic_get_volume(1),
                     mic_get_volume(2), mic_get_volume(3));
