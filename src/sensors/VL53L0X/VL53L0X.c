...
//////////////////// PUBLIC FUNCTIONS /////////////////////////
static THD_WORKING_AREA(waVL53L0XThd, 1024);
...
VL53L0X_Error VL53L0X_configAccuracy(VL53L0X_Dev_t *device, VL53L0X_AccuracyMode accuracy) {
    ...
    case VL53L0X_LONG_RANGE:
        ...
        if (status == VL53L0X_ERROR_NONE) {
            status = VL53L0X_SetVcselPulsePeriod(device, VL53L0X_VCSEL_PERIOD_PRE_RANGE, 18);
        }
        if (status == VL53L0X_ERROR_NONE) {
            status = VL53L0X_SetVcselPulsePeriod(device, VL53L0X_VCSEL_PERIOD_FINAL_RANGE, 14);
        }
        break;
...
void VL53L0X_start(void) {
    ...
    distThd = chThdCreateStatic(waVL53L0XThd, sizeof(waVL53L0XThd), NORMALPRIO + 10, VL53L0XThd, NULL);
}
...
uint16_t VL53L0X_get_dist_mm(void) { return dist_mm; }
